...
#if DEBUG
#define CHECK_FOR_LH_ImageCache_PLIST() if([key isEqualToString:@"LH_ImageCache.plist"]) { \
NSLog(@"LH_ImageCache.plist is a reserved key and can not be modified."); \
return; }
#else
#define CHECK_FOR_LH_ImageCache_PLIST() if([key isEqualToString:@"LH_ImageCache.plist"]) return;
#endif
...
static NSString* _LH_ImageCacheDirectory;

static inline NSString* LH_ImageCacheDirectory() 
{
	if(!_LH_ImageCacheDirectory) 
    {
		NSString* cachesDirectory = [NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) objectAtIndex:0];
		_LH_ImageCacheDirectory = [[[cachesDirectory stringByAppendingPathComponent:[[NSProcessInfo processInfo] processName]] stringByAppendingPathComponent:@"LH_ImageCache"] copy];
	}
	
	return _LH_ImageCacheDirectory;
}

static inline NSString* cachePathForKey(NSString* key) 
{
	return [LH_ImageCacheDirectory() stringByAppendingPathComponent:key];
}

static LH_ImageCache* __instance;

@interface LH_ImageCache ()
...
@implementation LH_ImageCache
@synthesize defaultTimeoutInterval;

+ (LH_ImageCache*)currentCache {
	@synchronized(self) {
		if(!__instance) {
			__instance = [[LH_ImageCache alloc] init];
...
- (id)init {
	if((self = [super init])) {
		NSDictionary* dict = [NSDictionary dictionaryWithContentsOfFile:cachePathForKey(@"LH_ImageCache.plist")];
		...
		[[NSFileManager defaultManager] createDirectoryAtPath:LH_ImageCacheDirectory() 
...
- (void)saveCacheDictionary {
	@synchronized(self) {
		[cacheDictionary writeToFile:cachePathForKey(@"LH_ImageCache.plist") atomically:YES];
	}
}
