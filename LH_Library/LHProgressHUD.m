...
- (void)setMode:(LHProgressHUDMode)newMode {
    ...
    {
        [self performSelector:@selector(updateIndicators)];
        [self performSelector:@selector(setNeedsLayout)];
...
- (void)setLabelText:(NSString *)newText {
	if ([NSThread isMainThread]) {
        ...
        [self performSelector:@selector(updateLabelText:) withObject:newText];
        [self performSelector:@selector(setNeedsLayout)];
        [self performSelector:@selector(setNeedsDisplay)];
...
- (void)setDetailsLabelText:(NSString *)newText {
	if ([NSThread isMainThread]) {
        ...
        [self performSelector:@selector(updateDetailsLabelText:) withObject:newText];
        [self performSelector:@selector(setNeedsLayout)];
        [self performSelector:@selector(setNeedsDisplay)];
...
- (void)setProgress:(float)newProgress {
    ...
    if (mode == LHProgressHUDModeDeterminate) {
		if ([NSThread isMainThread]) {
			[self updateProgress];
			[self setNeedsDisplay];
		} else {
...
- (void)updateIndicators {
    ...
    else if (mode == LHProgressHUDModeCustomView && self.customView != nil){
...
+ (LHProgressHUD *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	LHProgressHUD *hud = [[LHProgressHUD alloc] initWithView:view];
	[view addSubview:hud];
	[hud show:animated];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated 
{
	...
	{
		if ([v isKindOfClass:[LHProgressHUD class]]) 
		{
			viewToRemove = v;
		}
	}
	if (viewToRemove != nil) {
		LHProgressHUD *HUD = (LHProgressHUD *)viewToRemove;
...
- (void)viewDidHideThisViewInView:(UIView *)view
{
    [self performSelector:@selector(hideAction:) withObject:view];
	//[self performSelector:@selector(hideAction:) withObject:view afterDelay:0.5f];
}

- (void)hideAction:(UIView *)view
{
	UIView *viewToRemove = nil;
	for (UIView *v in [view subviews]) 
	{
...
- (id)initWithView:(UIView *)view {
	...
	if (!view) {
		[NSException raise:@"LHProgressHUDViewIsNillException" 
					format:@"The view used in the LHProgressHUD initializer is nil."];
	}
    CGRect frame = view.bounds;
    if(frame.size.height > 416)
    {
        frame.origin.y = frame.size.height - 416;
        frame.size.height = 416;
    }
	id me = [self initWithFrame:frame];
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
        ...
        self.animationType = LHProgressHUDAnimationFade;
        self.mode = LHProgressHUDModeIndeterminate;
        self.labelText = nil;
        ...
        if(myFrame.size.height > 416)
        {
            myFrame.origin.y = myFrame.size.height - 416;
            myFrame.size.height = 416;
        }
        // Add label
        label = [[UILabel alloc] initWithFrame:myFrame];
		
        // Add details label
        detailsLabel = [[UILabel alloc] initWithFrame:myFrame];
...
- (void)done {
    ...
    if(delegate != nil && [delegate conformsToProtocol:@protocol(LHProgressHUDDelegate)]) {
...
- (void)showUsingAnimation:(BOOL)animated {
    ...
    if (animated) {
        ...
        self.alpha = 1.0;
        if (animationType == LHProgressHUDAnimationZoom) {
...
- (void)drawRect:(CGRect)rect {
    ...
    if(allRect.size.height > 416)
    {
        allRect.origin.y = allRect.size.height - 416;
        allRect.size.height = 416;
    }
